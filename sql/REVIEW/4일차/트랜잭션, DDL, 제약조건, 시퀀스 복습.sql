...
세션
 ...
 -  DML(insert, update, delete)을 통해 작업시 세션A가  '테이블' or 'row'를 작업중이면 해당 작업은 'lock'이 걸린다.
...
서브쿼리
 - 항상 메인쿼리 보다 '먼저' 실행된다.
 - 조인과는 다른 방식으로 실행.
 - 서브쿼리는 상관 서브쿼리, 비상관 서브쿼리, 스칼라 서브쿼리
 
rownum
 - 행의 순서를 나타내는 가상의 번호.
 - 실제 컬럼으로 지정하지 않은 이상 존재하지 않음
 - 쿼리 결과에따라 순서대로 지정
 - 'order by보다 먼저 실행됨.'
 - '서브쿼리로 정렬' 후 rownum를 지정해주는 것이 좋음.
...
- 기본 키, 외래 키, 유니크 제약 조건은 데이터 무결성을 보장하는 제약 조건
1. not null
...
데이터 사전
 ...
 user_ : 사용자가 소유한 정보
 All_ : 접속한 사용자나 다른 사용자가 소유한 객체 중 허가받은 객체
